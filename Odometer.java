...
public class Odometer implements Runnable {
  ...
  double Left_disp,Right_disp;
  int prev_LeftCount,prev_RightCount; 
  ...
  public void run() {
    ...
    while (true) {
     ...
     //  the two sentences below are used in order to calculate the new values of the tacho counts. 
      leftMotorTachoCount = leftMotor.getTachoCount();
      rightMotorTachoCount = rightMotor.getTachoCount();
      
      double dX, dY, dDisplacement;
      double dTheta, dRadian;
      // TODO Calculate new robot position based on tachometer counts
     
      //The two sentences below are used to calculate the left and right displacements respecitvely.
      Left_disp = Math.PI*WHEEL_RAD*(leftMotorTachoCount - prev_LeftCount)/180;
      Right_disp = Math.PI*WHEEL_RAD*(rightMotorTachoCount - prev_RightCount)/180;
      
      //Updates the value of the tachoCount
      prev_LeftCount = leftMotorTachoCount;
      prev_RightCount = rightMotorTachoCount;
      
      //The two sentences below are used to calculate the angle.
      dDisplacement = (Left_disp+Right_disp)/2;
      dRadian = (Left_disp-Right_disp)/TRACK;
     
      if(x<0.0 && y<1)
      //Change in displacement and angle
      dX = dDisplacement * Math.sin(Math.toRadians(theta));
      dY = dDisplacement * Math.cos(Math.toRadians(theta));
      dTheta = dRadian*180/Math.PI;
